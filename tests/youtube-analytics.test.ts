...
import { fetchVideoStats, normalizeVideoStats } from "../src/youtube/analytics.ts";
...
test("normalizes malformed provider statistics to zero", () => {
  assert.deepEqual(normalizeVideoStats({ viewCount: "bad", likeCount: -3, commentCount: "7" }), { views: 0, likes: 0, comments: 7 });
});
