...
export type ChatOptions = {
  /**
   * A parameter rather than config, because the paid guard depends on it: a
   * signature without it would silently drop the check that stops an unattended
   * spend.
   */
...
/**
 * The transport shared by every caller of an OpenAI-compatible endpoint: Ollama,
 * LM Studio, llama.cpp, vLLM, OpenAI, DeepSeek, Groq, and OpenRouter. Moving
 * between them is a change of baseUrl and key, not of code. Prompt building and
 * response parsing stay with the caller — this returns the raw message content.
 */
